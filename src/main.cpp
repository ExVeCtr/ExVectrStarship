...
void attitudeTelemetryCallback(const Core::Timestamped<Math::Vector<float, 7>> &data)
{
    ...
    Net::PacketAttitude packet({data.data(0), data.data(1), data.data(2)}, {data.data(3), data.data(4), data.data(5), data.data(6)});
...
void positionTelemetryCallback(const Core::Timestamped<Math::Vector<float, 6>> &data)
{
    ...
    float hAcc = sqrt(posEstTask.getCovariance()(3, 3) * posEstTask.getCovariance()(3, 3) + posEstTask.getCovariance()(4, 4) * posEstTask.getCovariance()(4, 4)) / 1000;
    float vAcc = posEstTask.getCovariance()(5, 5) / 1000;

    Net::PacketPosition packet({data.data(3), data.data(4), data.data(5)}, {data.data(0), data.data(1), data.data(2)}, hAcc, vAcc);
...
void gnssTelemetryCallback(const Core::Timestamped<SNSR::GNSSData> &data)
{
    ...
    Net::PacketGPS packet(data.data.velocity,
                          data.data.position(0), data.data.position(1), data.data.position(2),
                          data.data.numSats,
                          data.data.positionCov(0, 0), data.data.positionCov(2, 2), data.data.velocityCov(0, 0));
...
public:
    ...
    void taskThread() override
    {
        ...
        if (magSubr.size() > 0)
        {
            ...
            magScale_ = sqrt(beta(3) + magBias_(0) * magBias_(0) + magBias_(1) * magBias_(1) + magBias_(2) * magBias_(2)) / 0.05; // Get the scale from the beta vector
...
MissionRTH defaultMission_(attitudeTopicSwitch.getTopic(), positionTopicSwitch.getTopic(), groundTopicSwitch.getTopic(), posEstTask.getGroundDistanceCovTopic(), {0, 0, 1.5}); // The default mission is the return to home mission.
...
public:
    ...
    void taskInit() override
    {
        ...
        missionWaypointTask.addWaypoint({0, 5, 2}, 1, 0.2, 0.2, 0.2, 0, 10 * Core::SECONDS);                      // Add a waypoint to the mission waypoint task
        missionWaypointTask.addWaypoint({0, 5, 120}, 7, 3, 2, 3, 5 * Core::SECONDS);                              // Add a waypoint to the mission waypoint task
        missionWaypointTask.setNextMission(&missionBellyflop);
        missionBellyflop.setNextMission(&missionFreefallTask); // Set the next mission of the belly flop mission to the free fall mission
        missionFreefallTask.setNextMission(&defaultMission_);  // Set the next mission of
    }
    ...
    void handleVehicleMissionControl()
    {
        ...
        else if (mission_->missionEnd() && (mission_->nextMission() == &defaultMission_ || mission_->nextMission() == nullptr))
        {                                                              // Mission has ended and we are not in the default mission (RTH), then we switch to RTH mission.
        ...
        else if (mission_->missionEnd() && mission_->nextMission() != nullptr)
        {                                                           // If the mission has ended and there is a next mission, then we switch to the next mission.
    ...
    void handleTelecommands()
    {
        ...
        case TelecommandType::Telecommand_CalibApply:
            if (telecommand.subsystem == Subsystem::Subsystem_Magneto)
            {
                ...
                if (memoryManager.writeItem(magTransform, MEMORY_KEY_MAGCALIB))
                { // Save the mag transform matrix to memory
                ...
                Math::Matrix<float, 3, 3> sensorToBody_ = {
                    ...
                    0, 1, 0,
                    0, 0, 1};
                if (!memoryManager.readItem(sensorToBody_, MEMORY_KEY_MAGTRANSFORM))
                {                                                                                           // Read the mag transform matrix from memory
        ...
        case TelecommandType::Telecommand_SimulationMode:
            ...
            { // Validate.
                ...
                if (simulationMode_)
                {
                    ...
                    groundTopicSwitch.subscribe(bodySimulator.getGroundTopic());        // Subscribe to the ground distance topic of the body simulator
                ...
                else
                {
                    ...
                    groundTopicSwitch.subscribe(posEstTask.getGroundDistanceEstTopic()); // Subscribe to the ground distance topic of the position estimator
    ...
    bool vehicleIsReady()
    {
        ...
        if (
            allSystemsInitialised_ && sensoryState_.imu == TelemetrySensor::TelemetrySensor_Ready && sensoryState_.baro == TelemetrySensor::TelemetrySensor_Ready && sensoryState_.gnss == TelemetrySensor::TelemetrySensor_Ready && sensoryState_.attitudeKF == TelemetrySensor::TelemetrySensor_Ready && sensoryState_.positionKF == TelemetrySensor::TelemetrySensor_Ready && !failureState_.sensorFailure && !failureState_.attitudeFailure && !failureState_.positionFailure && !failureState_.positionOutOfBounds && !failureState_.attitudeOutOfBounds && vehicleMode_ != VehicleMode::VehicleMode_Failure && vehicleMode_ != VehicleMode::VehicleMode_SensorCalib)
        {

            if (
                vehicleMode_ == VehicleMode::VehicleMode_Startup || vehicleMode_ == VehicleMode::VehicleMode_Failure)
            {
...
public:
    ...
    void taskThread() override
    {
        ...
        vehicleState.groundDistance = posEstTask.getGroundDistanceEst()(1) / 2 * INT16_MAX;
        vehicleState.groundDistanceCov = posEstTask.getGroundDistanceCov()(1, 1) * INT16_MAX;
...
void initialiseMemory()
{
    ...
    if (false)
    {
        ...
        magCalibData.cov = magCalibData.cov * Math::Matrix<float, 3, 3>({0, 0, -1,
                                                                         1, 0, 0,
                                                                         0, -1, 0});
        // memoryManager.writeItem(magCalibData, MEMORY_KEY_MAGCALIB);
