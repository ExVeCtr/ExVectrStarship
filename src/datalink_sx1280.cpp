...
namespace VCTR
{
    ...
    namespace Net
    {
        ...
        Datalink_SX1280::Datalink_SX1280(int nssPin, int nresetPin, int rfbusyPin, int dio1Pin, int txEnPin, int rxEnPin) : Task_Periodic("Datalink_SX1280", 10 * Core::MILLISECONDS)
        {
        ...
        void Datalink_SX1280::taskInit()
        {
            ...
            setPaused(false); // Pause the thread until dio interrupt
        ...
        void Datalink_SX1280::taskCheck()
        {
            ...
            if (digitalRead(DIO1_PIN_) == HIGH || (transmitBuffer_.size() > 0 && radioState_ == RadioState::Idle))
            { // If the dio pin is high or we have data to send, then we should run the task
                setPaused(false);
                setRelease(0);
            }
        }
        ...
        void Datalink_SX1280::taskThread()
        {
            ...
            if (digitalRead(DIO1_PIN_) == HIGH)
            {
                ...
                { // We have detected a preamble. This means we are receiving data.
            ...
            if (transmitBuffer_.size() > 0 && (radioState_ == RadioState::Idle || radioState_ == RadioState::Receiving) && threadTime - transmitEnd_ > 1 * Core::MILLISECONDS)
            { // If we have data to send and are currently not doing anything. Lets check if we can send.
            ...
            if (radioState_ == RadioState::Idle)
            { // Looks like we went into idle state. We should start receiving data.
            ...
            if (radioState_ == RadioState::Receiving && threadTime - receiveStart_ > 500 * Core::MILLISECONDS)
            { // Timout case for receiving data. We should start receiving data again.
            ...
            if (radioState_ == RadioState::Transmitting && threadTime - transmitStart_ > 500 * Core::MILLISECONDS)
            { // Timout case for transmitting data. Go into idle as something went wrong
            ...
            if ((radioState_ == RadioState::ChannelBusy || radioState_ == RadioState::ActivityDetection) && threadTime - channelBusyStart_ > 500 * Core::MILLISECONDS)
            { // Timout case for channel busy. Go into idle as something went wrong
        ...
        bool Datalink_SX1280::transmitAwaitingData()
        {
            ...
            while (transmitBuffer_.size() > 0 && bufferSize + transmitBuffer_[0] + 2 <= packetLimit && transmitBuffer_[0] != 0)
            { // The next addition of bytes would be the frame size plus also the frame size number. This number is needed by receiver to decode. An addistion byte for the crc.
        ...
        void Datalink_SX1280::receiveAwaitingData()
        {
            ...
            { // make sure packet is okay
                ...
                uint8_t crc = 0;
                for (size_t i = 0; i < packetL - 1; i++)
                    crc += bufferArray[i];
                uint8_t crcRcv = bufferArray[packetL - 1];

                if (crc == crcRcv)
                { // Only decode if crc is correct.
