...
private:
  struct Waypoint {
    Math::Vector<float, 3>
        position;     // The position of the waypoint to travel to.
    float velocity;   // The velocity to move to the waypoint in m/s.
    float startAccel; // The acceleration limit to move the waypoint in m/s^2 at
                      // the start of the waypoint travel. This is used to limit
                      // the acceleration/deceleration of the vehicle.
    float stopAccel;  // The deceleration limit to stop at the waypoint in m/s^2
                     // at the end of the waypoint travel. This is used to limit
                     // the acceleration/deceleration of the vehicle.
    float thresholdDistance; // When the waypoint can be considered as reached
                             // if within this distance.
    int64_t loiterTime; // The time the vehicle should loiter at the waypoint in
                        // seconds.
    int64_t timeLimit; // If the vehicle takes more than this time, the waypoint
                       // is considered as not reachable and the next waypoint
                       // is selected.
    bool
        cancelIfNotReachable; // If the waypoint was not reached within the time
                              // limit, the mission will be cancelled and ended.
  };
  ...
  int64_t lastUpdateTimestamp_ =
      0;                      // Timestamp of the last update in microseconds.
  ...
  size_t currentWaypointIndex_ =
      0; // The index of the current waypoint to travel to.
  int64_t waypointThresholdTime_ =
      0; // The time the vehicle should be within the threshold distance to the
         // waypoint to be considered as reached.
  int64_t waypointStartTime_ =
      0; // The time the vehicle started to travel to the waypoint.
  bool waypointReached_ =
      false; // If the waypoint has been reached or not. This is set to true
             // when the vehicle was within the threshold distance to the
             // waypoint.
...
public:
  ...
  /**
   * Adds a waypoint to the list of waypoints to travel to. Waypoint is added to
   * the end of the path
   * @param position The position of the waypoint to travel to.
   * @param velocity The velocity to move/lerp to the waypoint in m/s. Set to 0
   * for an instantaneous move. The velocity output will remain 0.
   * @param acceleration The acceleration limit to move the waypoint in m/s^2.
   * This is used to limit the acceleration/deceleration of the vehicle.
   * @param thresholdDistance When the waypoint can be considered as reached if
   * within this distance.
   * @param loiterTime The time the vehicle should loiter at the waypoint in
   * seconds.
   * @param timeLimit If the vehicle takes more than this time, the waypoint is
   * considered as not reachable and the next waypoint is selected. If not
   * given, the vehicle will calculate the time needed +50% to reach the
   * waypoint.
   * @param cancelIfNotReachable If the waypoint was not reached within the time
   * limit, the mission will be cancelled and ended.
   *
   */
  void addWaypoint(const Math::Vector<float, 3> &position, float velocity = 0,
                   float startAccel = 3, float stopAccel = 3,
                   float thresholdDistance = 1, int64_t loiterTime = 0,
                   int64_t timeLimit = Core::END_OF_TIME,
                   bool cancelIfNotReachable = false) {

    if (timeLimit ==
        Core::END_OF_TIME) { // If the time limit is not given, calculate the
                             // time needed to reach the waypoint

      if (waypoints_.size() > 0) {
        auto lastWaypointPos = waypoints_[waypoints_.size() - 1].position;
        auto distance =
            (position - lastWaypointPos)
                .magnitude(); // Calculate the distance to the waypoint
        timeLimit = (distance / velocity) * 1.5 *
                    Core::SECONDS; // Calculate the time needed to reach the
                                   // waypoint +50%
      } else {
        auto distance =
            position.magnitude(); // Calculate the distance to the waypoint
        timeLimit = (distance / velocity) * 1.5 *
                    Core::SECONDS; // Calculate the time needed to reach the
                                   // waypoint +50%
      }
    ...
    waypoints_.append(
        {position, velocity, startAccel, stopAccel, thresholdDistance,
         loiterTime, timeLimit,
         cancelIfNotReachable}); // Add the waypoint to the list of waypoints
  }
  ...
  void taskInit() override {
    ...
    lastUpdateTimestamp_ =
        Core::NOW(); // Set the last update timestamp to the current time
  }
  ...
  void beginMission(int64_t startTime) override {
    ...
    waypointStartTime_ =
        Core::NOW(); // Set the time when the waypoint was started
...
private:
  ...
  void missionInit() {
    ...
    if (missionTime_.NOW() >
        -10 * Core::SECONDS) { // If the mission time is greater than -5
                               // seconds, we consider it as initialised
      missionState_.missionMode =
          MissionMode::MissionMode_Startup; // Go to startup mode
    }
  }
  ...
  void missionStartup() {
    ...
    positionSetpoint_(3) = posSubr_.getItem().data(
    ...
    currentWaypointIndex_ = 0; // Reset the waypoint index
    if (waypoints_.size() == 0) {
      addWaypoint({0, 0, 1}, 0.5,
                  1); // Add a default waypoint to the list of waypoints
      LOG_MSG(
          "No waypoints set, adding default waypoint at (0, 0, 1)\n"); // Log
                                                                       // the
                                                                       // mission
                                                                       // start
    }
  ...
  void missionHover() {
    ...
    auto waypoint = waypoints_[currentWaypointIndex_]; // Get the current
                                                       // waypoint to travel to
    auto distance =
        waypoint.position -
        positionIs_.block<3, 1>(
            3); // Get the distance to the waypoint in reference frame
    auto distanceMagnitude =
        distance.magnitude(); // Get the magnitude of the distance vector

    if (distanceMagnitude <
        waypoint.thresholdDistance) { // If the vehicle is within the threshold
                                      // distance of the waypoint, we consider
                                      // it as reached

      waypointReached_ = true; // Set the waypoint reached to true

      if (Core::NOW() - waypointThresholdTime_ >
          waypoint
              .loiterTime) { // If the vehicle is within the threshold distance
                             // for a certain time, we consider it as reached

        positionSetpoint_(0) = 0; // Set the velocity setpoint to 0
        positionSetpoint_(1) = 0;
        positionSetpoint_(2) = 0;
        positionSetpoint_(3) = waypoint.position(
            0); // Set the setpoint position in the reference frame
        ...
        if (currentWaypointIndex_ < waypoints_.size() - 1) {
          ...
          LOG_MSG("Waypoint position: %.2f %.2f %.2f\n", waypoint.position(0),
                  waypoint.position(1), waypoint.position(2));
      ...
      waypointThresholdTime_ =
          Core::NOW(); // Set the time when the waypoint was reached

      if (!waypointReached_ &&
          Core::NOW() - waypointStartTime_ >
              waypoint.timeLimit) { // If the vehicle is within the threshold
                                    // distance for a certain time, we consider
                                    // it as not reachable
        ...
        if (waypoint
                .cancelIfNotReachable) { // If the waypoint was not reached
                                         // within the time limit, the mission
                                         // will be cancelled and ended
          LOG_MSG(
              "Waypoint not reachable. Mission cancelled\n"); // Log the mission
                                                              // cancelled
          missionEnd_ = true; // Set the mission end to true
        } else if (currentWaypointIndex_ <
                   waypoints_.size() - 1) { // If we are at the last waypoint,
                                            // we go to idle mode
          ...
          LOG_MSG(
              "Waypoint not reachable. Waypoint mission finished\n"); // Log the
                                                                      // mission
                                                                      // finished
          missionEnd_ = true; // Set the mission end to true
        }
    ...
    // Calculate the velocity limit to decelerate at the given amount and stop
    // at the waypoint position
    ...
    if (waypoint.stopAccel > 0.001 && currentVelocity_ > accelVelocityLimit) {
      currentVelocity_ = accelVelocityLimit;
    } else if (waypoint.velocity > currentVelocity_) {
      currentVelocity_ += waypoint.startAccel * dTime;
      if (currentVelocity_ > waypoint.velocity || waypoint.startAccel < 0.001) {
        currentVelocity_ = waypoint.velocity;
      }
    }

    auto travelDistance = waypoint.position - positionSetpoint_.block<3, 1>(3);
    auto travelDistanceNorm = travelDistance.normalize();
    if (travelDistance.magnitude() / dTime > currentVelocity_) {
      travelDistance = travelDistance.normalize() * currentVelocity_ * dTime;
    }
    positionSetpoint_(0) = travelDistanceNorm(0) * currentVelocity_;
    positionSetpoint_(1) = travelDistanceNorm(1) * currentVelocity_;
    positionSetpoint_(2) = travelDistanceNorm(2) * currentVelocity_;
    positionSetpoint_(3) += travelDistance(0);
