...
namespace VCTR
{
    ...
    private:
        ...
        Core::Simple_Subscriber<Core::Timestamped<Math::Vector<float, 2>>> groundSubr_;
        Core::Simple_Subscriber<Core::Timestamped<Math::Matrix<float, 2, 2>>> groundCovSubr_;
    ...
    public:
        MissionRTH(Core::Topic<Core::Timestamped<Math::Vector<float, 7>>> &attTopic, Core::Topic<Core::Timestamped<Math::Vector<float, 6>>> &posTopic, Core::Topic<Core::Timestamped<Math::Vector<float, 2>>> &groundTopic, Core::Topic<Core::Timestamped<Math::Matrix<float, 2, 2>>> &groundCovTopic, const Math::Vector<float, 3> &homePosition = {0, 0, 0}) : Task_Periodic("Mission RTH", 0.1 * Core::SECONDS)
        {
    ...
    private:
        ...
        void missionHover()
        {
            ...
            case RunningState::RunningState_Stabilize:
                ...
                if (posSubr_.getItem().data.block<3, 1>(0, 0).magnitude() < 0.5)
                {                                                      // If the vehicle is slower than 0.5 m/s, we consider it as stopped, and begin translation
...

            {
                ...
                if (translationVelocity_ > 2)
                { // Limit the translation velocity to 2 m/s
                ...
                {                                                       // If the vehicle is within 0.5 m of the home position, we begin descent
        ...
        void missionDescent()
        {
            ...
            LOG_MSG("Ground distance: %.2f, Covariance: %.2f\n", groundDistance, groundCov); // Log the ground distance and covariance

            if (positionIs_(5) - positionSetpoint_(5) < landingThresholdDistance_ * 1.5)
            { // We keep updateting the threshold time. We stop when the vehicle is above the threshold distance. This triggers the start of the timer.
            ...
            if (positionIs_(5) - positionSetpoint_(5) < landingThresholdDistance_)
            {                                       // We keep updateting the threshold time. We stop when the vehicle is above the threshold distance. This triggers the start of the timer.
                landingThresMetTime_ = Core::NOW(); // Set the time when the landing threshold was met
            }

            if (Core::NOW() - landingThresMetTime_ > landingThresholdTime_)
            {                                                                  // If the vehicle is below the landing threshold distance for a certain time, we consider it as landed.
                missionState_.missionMode = MissionMode::MissionMode_Finished; // Go to landed mode
            }

            if (groundDistance < 0.04 && groundDistance > -0.02 && groundCov < 7 && Core::NOW() - groundLastUpdate < 100 * Core::MILLISECONDS) // If the ground distance is less than 4 cm and is confident, we consider it as landed.
            {
                missionState_.missionMode = MissionMode::MissionMode_Finished; // Go to landed mode
            }
