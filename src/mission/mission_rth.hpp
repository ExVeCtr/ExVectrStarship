...
class MissionRTH : public MissionAbstract, public Core::Task_Periodic {
 ...
 public:
  MissionRTH(
    ...
    homePosition_ =
        homePosition;  // Set the home position to the given position
  ...
  void beginMission(int64_t startTime) override {
    ...
    LOG_MSG(
        "Started mission ReturnToHome. Hot starting the mission immedietly\n");  // Log the mission start
    missionStartup();
  };
 ...
 private:
    ...
    switch (runningState_) {
      case RunningState::RunningState_Stabilize:
        ...
        positionSetpoint_(3) = posSubr_.getItem().data(
            3);  // Set the position setpoint to where the vehicle is. Keep this
                 // updated as we only want to stop the vehicle.
        ...
        if (posSubr_.getItem().data.block<3, 1>(0, 0).magnitude() <
          ...
          runningState_ =
              RunningState::RunningState_Ascent;  // Go to ascent mode
      ...
      {
        auto travelDistance =
            homePosition_ - positionSetpoint_.block<3, 1>(
                                3);  // Get the velocity vector to the home
        ...
        if (translationVelocity_ < 2) translationVelocity_ += dTime * accel;
        ...
        if (distance.magnitude() <
            0.5) {  // If the vehicle is within 0.5 m of the home position, we
                    // begin descent
          runningState_ =
  ...
  void missionDescent() {
    ...
    positionSetpoint_(3) =
        homePosition_(0);  // Set the position setpoint to where the vehicle is
    positionSetpoint_(4) =
        homePosition_(1);  // Set the position setpoint to where the vehicle is
